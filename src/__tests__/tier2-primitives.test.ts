...
import { createDailyBudget, createCircuitBreaker } from "../rate.ts";
import { escapeHtml, normalizeSearchText, toAlphanumeric } from "../text.ts";
import { parseGitHubRepoInput, createGitHubClient } from "../github.ts";
import { assertRequiredEnvironment } from "../environment.ts";
...
describe("retry shouldRetry predicate", () => {
  it("stops immediately when shouldRetry rejects the error", async () => {
    ...
    await expect(
      retry(action, { retries: 3, delay: 1, shouldRetry: () => false }),
  ...
  it("retries while shouldRetry accepts", async () => {
    ...
    await expect(
      retry(action, { retries: 3, delay: 1, shouldRetry: (e) => (e as Error).message === "transient" }),
    ).resolves.toBe("ok");
...
describe("assertRequiredEnvironment", () => {
  it("aggregates all missing values into one error", () => {
    expect(() =>
      assertRequiredEnvironment({ A: "set", B: "", C: undefined }),
    ).toThrow("[config] Missing required environment variable(s): B, C");
