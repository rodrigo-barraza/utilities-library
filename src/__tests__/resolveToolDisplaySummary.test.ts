...
describe("resolveToolDisplaySummary", () => {
  ...
  describe("display metadata with descriptionParam", () => {
    ...
    it("uses toolLabel + description when description is present", () => {
      const result = resolveToolDisplaySummary(
        "execute_command",
        { command: "ls -F", description: "List files in current directory" },
        { display },
      );
      expect(result).toEqual({
        verb: "Bash",
    ...
    it("falls back to verbs + subjectParam when description is absent", () => {
      const completed = resolveToolDisplaySummary(
        "execute_command",
        { command: "ls -F" },
        { display },
      );
      ...
      const active = resolveToolDisplaySummary(
        "execute_command",
        { command: "ls -F" },
    ...
    it("falls back to verbs when toolLabel is not set", () => {
      const result = resolveToolDisplaySummary(
        "execute_command",
        { command: "ls -F", description: "List files" },
        { display: { ...display, toolLabel: undefined } },
      );
