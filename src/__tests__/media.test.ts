...
import { rewritePrivateUrls, rewriteStream } from "../media.ts";

const MINIO = "http://192.168.1.100:9000";

async function collect(chunks: string[], internalUrl?: string): Promise<string> {
...
describe("rewriteStream", () => {
  it("rewrites a URL split across chunks", async () => {
    ...
    expect(await collect([body.slice(0, splitAt), body.slice(splitAt)], MINIO)).toBe(
      'data: {"url":"/api/media/discord-media/a.png"}\n\n',
    );
  });
