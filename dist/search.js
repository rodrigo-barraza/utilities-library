...
export class Bm25ToolIndex {
    ...
    toolNamesByDocument = new Map();
    constructor(documents) {
        this.totalDocuments = documents.length;
        let totalTokenCount = 0;
        for (const document of documents) {
            const searchableText = extractSearchableText(document);
     ...
     */
    computeBm25(documentTokens, queryTerms) {
        let score = 0;
        const documentLength = documentTokens.length;
        for (const queryTerm of queryTerms) {
            const termFrequency = documentTokens.filter((token) => token === queryTerm).length;
            if (termFrequency === 0)
                continue;
            const documentsContainingTerm = this.documentFrequency.get(queryTerm) || 0;
            // IDF: rarer terms across the corpus score higher
            const inverseDocumentFrequency = Math.log((this.totalDocuments - documentsContainingTerm + 0.5) /
                (documentsContainingTerm + 0.5) +
                1);
            // TF with saturation + length normalization
            const normalizedTermFrequency = (termFrequency * (BM25_TERM_FREQUENCY_SATURATION + 1)) /
                (termFrequency +
    ...
    search(query, limit = 20) {
        ...
        scoredResults.sort((firstResult, secondResult) => secondResult.score - firstResult.score);
